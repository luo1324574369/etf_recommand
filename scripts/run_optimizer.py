...
def load_data(db_path, codes):
    ...
    try:
        ...
        data = {}
        for code in codes:
            prices = repo.get_daily_price(code)
            if prices:
                data[code] = pd.DataFrame(prices)
        return data
    finally:
        db.close()
...
def main():
    parser = argparse.ArgumentParser(description="运行多因子策略参数优化")
    ...
    print(f"有效ETF: {len(data_dict)} | 参数组合: {result.get('total_combinations', 0)}")
    print(f"最优参数: {result.get('best_params', {})}")
    if result.get("best_result"):
        best = result["best_result"]
        print(
            f"收益={best.get('total_return', 0):.2f}% "
            f"夏普={best.get('sharpe_ratio', 0):.3f} "
            f"回撤={best.get('max_drawdown', 0):.2f}%"
        )
