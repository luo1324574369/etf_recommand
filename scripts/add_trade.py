...
from presentation.cli.render import blue, bold, green, red, gray, yellow


def get_latest_price(price_repo, code: str) -> float | None:
...
def do_buy(repo, etf_repo, price_repo):
    ...
    name = etf["name"]
    latest_price = get_latest_price(price_repo, code)
    price_hint = f"最新价: {latest_price:.2f}" if latest_price else "无最新价"
    print(f"  → {blue(code)} {bold(name)} {gray(price_hint)}")

    account = repo.get_account()
    if not account:
        console.error("请先初始化账户: python scripts/init_account.py")
        return

    qty = prompt_int("买入数量（股）")
    price = prompt_float("买入价格", default=latest_price)
    ...
    repo.execute_buy(code=code, quantity=qty, price=price, fee=fee, trade_date=trade_date)

    new_cash = account["cash"] - total
    console.success(f"买入记录已保存")
    print(f"  当前现金余额: {new_cash:.2f} 元")


def do_sell(repo, etf_repo, price_repo):
    code = input("请输入ETF代码: ").strip()
    ...
    latest_price = get_latest_price(price_repo, code) or 0
    ...
    price = prompt_float("卖出价格", default=latest_price)
    fee = prompt_float("手续费", default=5.0, min_val=0)
    ...
    trade_date = date.today().isoformat()
    repo.execute_sell(code=code, quantity=sell_qty, price=price, fee=fee, trade_date=trade_date)

    account = repo.get_account()
    remaining = qty - sell_qty
    console.success(f"卖出记录已保存")
...
def show_holdings(repo, etf_repo, price_repo):
    ...
    for h in holdings:
        ...
        etf = etf_repo.get_etf(code)
        name = etf["name"] if etf else code
        latest = get_latest_price(price_repo, code) or 0
        qty = h["quantity"]
        cost = h["cost_price"]
        mv = qty * latest
        profit = mv - qty * cost
        ...
        profit_color = green if profit >= 0 else red
        print(f"  {blue(code)} {bold(name)}: {qty}股 成本{cost:.2f} 现价{latest:.2f} 市值{mv:.0f} {profit_color(f'{profit_pct:.2%}')}")
    print()
...
def main():
    ...
    account = repo.get_account()
    if not account:
        console.warn("请先初始化账户: python scripts/init_account.py")
        conn.close()
        return
    ...
    while True:
        ...
        if choice == "quit":
            ...
            do_buy(repo, etf_repo, price_repo)
        elif choice == "卖出":
            do_sell(repo, etf_repo, price_repo)
        elif choice == "查看当前持仓":
            show_holdings(repo, etf_repo, price_repo)
